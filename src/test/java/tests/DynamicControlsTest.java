...
public class DynamicControlsTest extends BaseTest {
    ...
    public void shouldRemoveCheckboxAndEnableInput() {
        ...
        Assert.assertTrue(dynamicControlsPage.isCheckboxDisplayed(), "Checkbox should be visible at the start.");
        dynamicControlsPage.clickRemoveButton();
        dynamicControlsPage.waitForCheckboxToDisappear();

        Assert.assertEquals(dynamicControlsPage.getMessageText(), "It's gone!",
                "Unexpected remove checkbox message.");
        Assert.assertFalse(dynamicControlsPage.isCheckboxDisplayed(),
                "Checkbox should not be visible after remove action.");

        Assert.assertFalse(dynamicControlsPage.isInputEnabled(), "Input should be disabled at the start.");
        dynamicControlsPage.clickEnableButton();
        dynamicControlsPage.waitForInputToBeEnabled();

        Assert.assertEquals(dynamicControlsPage.getMessageText(), "It's enabled!",
                "Unexpected enable input message.");
